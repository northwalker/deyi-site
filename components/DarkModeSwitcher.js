...
export default function DarkModeSwitcher() {
  ...
  return (
    <div className="relative h-8">
      <button
        className="flex h-8 w-14 overflow-hidden rounded-full border border-black bg-[#F6F6F7] dark:border-text-color dark:bg-[#333333]"
        type="button"
        role="switch"
        aria-checked="true"
        onClick={changeMode}
      ...
      >
        <span className="absolute top-1/2 ml-1 h-6 w-6 -translate-y-1/2 rounded-full bg-white p-1 transition-all duration-500 dark:ml-7 dark:bg-black">
