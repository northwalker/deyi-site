...
export default function Page() {
  ...
  const handleEncode = useCallback(() => {
    try {
      const encodedText = window.btoa(decodeText);
  ...
  const handleDecode = useCallback(() => {
    try {
      const decodedText = window.atob(encodedText);
    ...
    <div className="relative mx-auto my-0 max-w-2xl px-4 pb-0 pt-4 text-center text-lg text-black dark:text-white md:pt-16">
      ...
      <section className="mt-8 flex flex-col">
        ...
        <div className="my-4 flex flex-row items-center justify-center">
          <MaterialButton className="mx-4" onClick={handleEncode}>
            Encode ↓
          </MaterialButton>
          <MaterialButton className="mx-4" onClick={handleDecode}>
            Decode ↑
          </MaterialButton>
        </div>
        <div>
